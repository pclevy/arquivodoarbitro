	...
	*/
	...
	if($ArqTorneio=='')
	{
		...
		if($versao_teste == true)
		{
			...
			//$ArqTorneio ="CRXpEq_Et1.TUMx" ;$remetente = "AF Paulo C. Levy";$datarec = '2012/05/28';
			//echo "*$ArqTorneio* - *$remetente* - *$datarec*";exit;
		}
			//$ArqTorneio ="ii_aberto_brasil_cordeiropolis_210290.TUNX" ;$remetente = "AF Paulo C. Levy";$datarec = '2014/09/27';//$apresentacao = "geral";
	...
	/*
		...
		*/
		...
		$TituloTorneio = LerCampoStr($fh);
		$SubTituloTorneio = trim(LerCampoStr($fh));
		$LeadTorneio = LerCampoStr($fh);
		$Diretor = LerCampoStr($fh);
		$Organizador = LerCampoStr($fh);
		$LocalTorneio = LerCampoStr($fh);
		$ArbitrosAux = LerCampoStr($fh);
		$ArqPGNentrada = LerCampoStr($fh);
		$ArqPGNsaida = LerCampoStr($fh);
		$TitTornPGN = LerCampoStr($fh);
		$LocTornPGN = LerCampoStr($fh);
		$ArquivoTUNx = LerCampoStr($fh);
		$ParamNIdent01 = LerCampoStr($fh);
		$CatIdades = LerCampoStr($fh);
		$Ritmo = LerCampoStr($fh);
		$ArquivoHTMLRod = LerCampoStr($fh);
		$ParamNIdent02 = LerCampoStr($fh);
		$ParamNIdent03 = LerCampoStr($fh);
		$ParamNIdent04 = LerCampoStr($fh);
		$ParamNIdent05 = LerCampoStr($fh);
		$PaisSediante = LerCampoStr($fh);
		$ArbitroPrincipal = LerCampoStr($fh);
		$FedOficial = LerCampoStr($fh);
		$EMail = LerCampoStr($fh);
		$HomePage = LerCampoStr($fh);
		$Pais2 = LerCampoStr($fh);
		...
		//echo "arbitro: $Arbitro  - $posPonteiro1 - $posPonteiro2!<br>";
	...
	//*** bloco de ajuste - 2012/12/14 ***** Inicio ******
		...
		//$Nulo=fread($fh, 124);
		$Nulo=fread($fh, 12);
		//echo TestarSeqAsc($fh,212) . '<br>';exit;
		//echo "? $Nulo ?";exit;
...
//$intervAdic=1;
  ...
  /*
	...
	*/
		...
		$Nulo=fread($fh, 10);			// *** verificado até aqui ***
	
		$TipoTorneio = LerCampoAsc($fh);
		
		$Nulo=fread($fh, 1);
		$DataDesc = LerCampoDat($fh);
  
		$Nulo=fread($fh, 2);
		$QtJogadores = LerCampoInt($fh);
  ...
  for($i=1; $i<10; $i++)
   {
		...
		$TodosTab = LerCampoAsc($fh);		// Equipe ???
  If ($TodosTab == 0)
		 {$DescrTodosTab = "Não";}
  Else
		 {$DescrTodosTab = "Sim";}
	...
	*/
	//echo "---" . $NumRodadas . "---";
		  ...
		  $z=1;
			while ($z<=$QtJogadores)
			{
				...
				*/
				...
				//$Clube[$z] = htmlspecialchars($Clube[$z], ENT_QUOTES);
			...
			*/				
			...
			//echo '<br><br>' . $MatrizGrupos['M'] . '<br><br>';
			$zg=0;
			...
			//echo '<br><br>';
				...
				//test43
				//echo '<div id="test43" style="visibility:hidden;display:inline-block;padding:0px;">.</div>';
        ...
        // **************************************************
								...
								// ***** ATENÇÃO: mudar a estrutura ***** 2016/0422 ********
								...
								switch ($CodRes)
									{
										...
										default:
											$ResulJogA = 0; $ResulJogB = 0;
											$PontosJogA = 0; $PontosJogB = 0;
											echo "Código Desconhecido encontrado: $CodRes - Rodada: $r - Tabuleiro: $MesaTabul!!";exit;	// verificar caso de "Double Round-Robin".
									}
           ...
           function MostrarTodosEmparceiramentos(acao)
            {
				      ...
				      if(mostrarListaJogadores=='hidden')
						...
						function FecharJanelas()
						{
							...
							divGrupoJogadores.style.visibility='hidden';divGrupoJogadores.style.height='0px';Grupo_Jogadores.style.height='0px';Grupo_Jogadores.style.visibility='hidden'; /*divMostrarGrupoJogadores.innerHTML='<a href=\"#\"><span onclick=\"MostrarJanela(divGrupoJogadores.id,divMostrarGrupoJogadores.id);\">Mostrar Grupos</span></a>&nbsp;';*/
