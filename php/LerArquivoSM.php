	...
	*/
	...
	if($ArqTorneio=='')
	{
		...
		if($versao_teste == true)
		{
			...
			$ArqTorneio ="2015intera_191006y5.TUMx" ;$remetente = "AF Paulo C. Levy";$datarec = '2022/01/24';
		...
		$fh = fopen($file, 'rb');
		//echo  '<br>' . TestarSeqAsc($fh,$TamArqTorneio) . '<br>';exit;
		//echo  '<br>' . TestarSeqAsc($fh,155) . '<br>';exit;
		...
		//echo  '<br>' . TestarSeqAsc($fh,47) . '<br>';exit;
	...
	/*
		...
		*/
		...
		// *** Campo Árbitros variando: verificar como identificar ***
		  ...
		  //echo ">".$ArbitrosAux."< <br>";
		//
		//echo  '<br>' . TestarSeqAsc($fh,1) . '<br>';//exit;
    ...
    //echo "<br>Pointer 1: ".ftell($fh)." <br>";
		...
		//echo  '<br>' . TestarSeqAsc($fh,17) . '<br>';exit;
    //echo "<br>Pointer 2: ".ftell($fh)." <br>";
      //$Nulo=fread($fh, 2); // ***** 2022/02/06 *****
			//echo  '<br>S: ' . TestarSeqAsc2($fh,104) . '<br>';//exit;
		//$ArquivoSMx = LerCampoStr($fh);echo "<br>Arq. Tunx: $ArquivoSMx <br>";
		//echo  '<br>' . TestarSeqAsc($fh,9) . '<br>';//exit;
    $ArquivoSMx = LerCampoStrLonga($fh); //echo "<br>Arq. Tunx: *****$ArquivoSMx*****<br>";//exit;
    //echo "<br>Pointer 3: ".ftell($fh)." <br>";

		//echo  '<br>' . TestarSeqAsc($fh,25) . '<br>';exit;
      //$Nulo=fread($fh, 2); // ***** 2022/02/06 *****
    ...
    //$Nulo=fread($fh, 29); // ***** 2022/02/06 *****
		...
		//echo  '<br>' . TestarSeqAsc2($fh,18) . '<br>';//exit;
		$ArquivoHTMLRod = LerCampoStr($fh); //echo "ArquivoHTMLRod: $ArquivoHTMLRod <br>";//exit;
		$ParamNIdent02 = LerCampoStr($fh); //echo "ParamNIdent02: $ParamNIdent02 <br>";
		$ParamNIdent03 = LerCampoStr($fh); //echo "ParamNIdent03: $ParamNIdent03 <br>";
		$ParamNIdent04 = LerCampoStr($fh); //echo "ParamNIdent04: $ParamNIdent04 <br>";
		$ParamNIdent05 = LerCampoStr($fh); //echo "ParamNIdent05: $ParamNIdent05 <br>";
		...
		//echo  '<br>' . TestarSeqAsc($fh,18) . '<br>';//exit;
		//echo "$NuloN0z1";exit;
    ...
    //$dados4 = fread($fh, 184);
		...
		//echo  '<br>' . TestarSeqAsc($fh,38) . '<br>';//exit;

    $dadosY = fread($fh, 37); // ***** 2022/03/18 *****
    $QtJogMesaEquip = LerCampoAsc($fh); // ***** 2022/03/18 *****
		//echo "<br>QtJogMesaEquip: $QtJogMesaEquip <br>";//exit;
...
//$intervAdic=1;
  ...
  Else
		...
		//echo "<br>$PtsPartEquipe<br>";
		
		$Nulo=fread($fh, 3);
		//echo '<br>' . TestarSeqAsc($fh, 3) . '<br>'; // 8
		...
		$Nulo=fread($fh, 558);
		//echo '<br>' . TestarSeqAsc($fh, 558) . '<br>';
		...
		*/
		...
		for($icrit=1;$icrit<=5;$icrit++)
		 {
				...
				$Nulo=fread($fh, 17);
				//echo '<br>' . TestarSeqAsc($fh, 17) . '<br>';
				
		 }
		//echo 'Tell: '.ftell($fh).'<br>';
		$Nulo=fread($fh, 130);
		//echo '<br>' . TestarSeqAsc($fh, 130) . '<br>';
		...
		$Nulo=fread($fh, 4949);
		//echo '<br>' . TestarSeqAsc($fh, 4949) . '<br>';
	...
	//echo "---" . $NumRodadas . "---";
		if($NumRodadas>0) {
			...
			//echo TestarSeqAsc($fh,912) . '***<br>';exit;
				...
				//echo '<br>' . TestarSeqAsc($fh, 4) . '<br>';

				//echo "Nulo: $Nulo";
			
			$RankInicial[0]=0;		// *** talvez seja eliminado ***
			$SobreNomeJogador[0] = '&nbsp;';
			$PreNomeJogador[0] = '&nbsp;';
			$NomeJogador[0] = '&nbsp;';
			$TitAcademico[0] = '&nbsp;';
			...
			$Clube[0] = '&nbsp;';
			$PaisJogador[0] = '&nbsp;';
			$Federacoes[0]=0; //$Federacao=RetirarNulo($PaisJogador[$z]);
			$Categoria[0] = '&nbsp;';
			$Grupo[0] = '&nbsp;';
			$Fonte[0] = '&nbsp;';
			$Sexo[0] = '&nbsp;';
			$RatFIDE[0] = 0;
			$RatNacional[0] = 0;
			$DataNasc[0] = ''; //LerCampoDat($fh);
			$NumeroClube[0] = 0; //LerCampoAsc($fh);
			$OrdInicJog[0] = 0;
			$IdFIDE[0] = 0; //LerCampoLong($fh);
			$Equipe[0] = 0; //LerCampoAsc($fh);
			$TabulEquip[0] = 0; //LerCampoAsc($fh);//echo "	NumTab: $Equipe[$z] / $TabulEquip[$z] <br>";
			$DM[0] = 0; //LerCampoAsc($fh);
			$Status[0] = 0; //ord(fread($fh,1));
			$PtsAdic[0] = 0;
			$BNoPair[0] = 0;
			$SNo[0] = 0;
			$FatorK[0] = 0;
      ...
      */
			...
			//echo '<br>'.$NumGrupos.'<br>';
				...
				{
				 $PontosEquipe[$e]=0;
				 for ($Rod=1;$Rod<=$NumRodadas;$Rod++)
				 {$PontosEquipeRod[$e][$Rod]=0;}
				 $QtJogadoresEquipe[$e]=0;
				}
				
				//for ($j=1;$j<=$QtJogadores;$j++) // ***** 2022/02/27 *****
				for ($j=0;$j<=$QtJogadores;$j++)
				{
					$PontosJogador[$j]=0;
					$JogClassif[$j]=0;
					$performace[$j]=0;
						...
						for ($p=$ptini;$p<=($ptini + 21 * $TabulRodada[$r] - 1);$p=$p+21)						
							{
							   ...
							   //echo $CodJogA." -a- ".$NomeJogador[$CodJogA]."<br>";
							  // ***** 2022/02/27 *****
								//echo "$CodJogA";exit;
								...
								//echo $CodJogB." -b- ".$NomeJogador[$CodJogB]."<br>"; // ***** 2022/02/27 *****
...
//								echo $CodJogA . ' ' . $CodJogB . ' ' . $CodRes . '<br>';
								//$JogB = $NomeJogador($CodJogB) & " (" & $CodJogB & ")"
         // **************************************************
         if($CodJogB==-2 & $CodRes>0) {$CodRes = $CodRes + 12;} // ***** 2022/02/27 *** ATENÇÃO *****
         // **************************************************
								...
								$RodMesaResult[$r][$MesaTabul][14]=$Equipe[$CodJogA]; // ***** 2022/03/05 *****
								//$PontosEquipeRod[$Equipe[$CodJogA]][$r]=$PontosEquipeRod[$Equipe[$CodJogA]][$r]+$PontosJogador[$CodJogA];
								...
								if($CodJogB<1) {
									...
									//$RodMesaResult[$r][$MesaTabul][15]=0; // ***** 2022/03/05 *****
								}
								else {
									...
									//$RodMesaResult[$r][$MesaTabul][15]=$Equipe[$CodJogB]; // ***** 2022/03/05 *****
								}
								$RodMesaResult[$r][$MesaTabul][15]=$Equipe[$CodJogB]; // ***** 2022/03/05 *****
								...
								$PontosEquipeRod[$Equipe[$CodJogA]][$r]=$PontosEquipeRod[$Equipe[$CodJogA]][$r]+$PontosJogA;

								if($CodJogB>0)
								{
									$PontosJogador[$CodJogB] = $PontosJogador[$CodJogB] + $PontosJogB;
									$PontosEquipeRod[$Equipe[$CodJogB]][$r]=$PontosEquipeRod[$Equipe[$CodJogB]][$r]+$PontosJogB;
								}
...
//			echo "<br><br><br> ... : " . ftell($fh) . "  --  " . $file . '= ' . filesize($file) . " bytes";

//      echo '<br>*' . ord(chr(0)).ord(chr(1)).'*-' . LerCampoIntTest(chr(0).chr(1)).'-*'. ord(chr(0)).ord(chr(8)).'*-' .LerCampoIntTest(chr(0).chr(8)) . '<br><br>';
//      echo '<br>*' . ord(chr(1)).'/'.ord(chr(44)).'*-' . LerCampoIntTest(chr(1).chr(44)).'<br><br>';

			$controle_chave =TestarSeqAsc($fh, 4);
//			echo '<br>' . $controle_chave . '<br><br>';
			//echo '<br>' . TestarSeqAsc($fh, filesize($file)-ftell($fh)) . '<br>';
			...
			for ($ir=1;$ir<=$NumRodadas;$ir++)
			{
				...
				$QtMesaRod[$ir]=0;
//				echo 'Rodada: '.$ir .'<br>';
				while ($NumEquipesJogando<$QtEquipes)
				{
					$MesaRod = LerSeqStr($fh, 15);
					$Mesa++;
					$QtMesaRod[$ir]++;
//					echo $Mesa .' - /';
					for($dm=0;$dm<15;$dm++)
...
//					echo '<br> &nbsp; &nbsp; &nbsp;&nbsp;';
					...
					$Equipe1 = ord($MesaRod[0]);
					$RodMesaEquip[$ir][$Mesa][1][0] = $Equipe1;	
					$RodMesaEquip[$ir][$Mesa][1][1] = $NomeEquipeR[$Equipe1];
					
					$Equipe2 = ord($MesaRod[2]);

					if ($Equipe2<=$QtEquipes)
					{
						$RodMesaEquip[$ir][$Mesa][2][0] = $Equipe2;
						$RodMesaEquip[$ir][$Mesa][2][1] = $NomeEquipeR[$Equipe2];
						//echo $Equipe1.'-"'.$NomeEquipeR[$Equipe1] . '" - ' . $Equipe2.'-"'.$NomeEquipeR[$Equipe2] . '"<br>';
						$NumEquipesJogando = $NumEquipesJogando + 2;
					}
					else
					{
						if ($Equipe2 == 255)
						{
							$RodMesaEquip[$ir][$Mesa][2][0] = ' ';
							$RodMesaEquip[$ir][$Mesa][2][1] = 'Bye';

							$PtsEquipBye = ord($MesaRod[4])/2;
							$PontosEquipeRod[$RodMesaEquip[$ir][$Mesa][1][0]][$ir]=$PtsEquipBye;

							//echo $Equipe1.'-"'.$NomeEquipeR[$Equipe1] . '" - ' . 'Bye' . ' <br>';
						 $NumEquipesJogando = $NumEquipesJogando + 1;
						}
						elseif ($Equipe2 == 254)
						{
							$RodMesaEquip[$ir][$Mesa][2][0] = ' ';
							$RodMesaEquip[$ir][$Mesa][2][1] = 'Ñ-Emparc.';
							//echo $Equipe1.'-"'.$NomeEquipeR[$Equipe1] . '" - ' . 'Ñ-Emparc.' . ' <br>';
						 $NumEquipesJogando = $NumEquipesJogando + 1;
						}
					}
					//echo $RodMesaEquip[$ir][$Mesa][1][0] . '-' . $RodMesaEquip[$ir][$Mesa][1][1] . ' (' . $PontosEquipe[$RodMesaEquip[$ir][$Mesa][1][0]] . ') -- ';
//					echo $RodMesaEquip[$ir][$Mesa][1][0] . '-' . $RodMesaEquip[$ir][$Mesa][1][1] . ' (' . $PontosEquipeRod[$RodMesaEquip[$ir][$Mesa][1][0]][$ir] . ') -- ';
					                                                                                    //  $PontosEquipeRod[$Equipe      [$CodJogA]        ][$r]
																																															//$RodMesaEquip[$ir][$Mesa][1][0]
					if ($Equipe2<=$QtEquipes)
					{
					 //echo $RodMesaEquip[$ir][$Mesa][2][0] . '-' . $RodMesaEquip[$ir][$Mesa][2][1] . ' (' . $PontosEquipe[$RodMesaEquip[$ir][$Mesa][2][0]] . ') <br>';
//					 echo $RodMesaEquip[$ir][$Mesa][2][0] . '-' . $RodMesaEquip[$ir][$Mesa][2][1] . ' (' . $PontosEquipeRod[$RodMesaEquip[$ir][$Mesa][2][0]][$ir] . ') <br>';
					}
					else
					{
//						echo $RodMesaEquip[$ir][$Mesa][2][1] . '<br>';
					}
 ...
 */
  ...
  echo '<style type="text/css">
	...
	echo "<script type='text/javascript'>PontosJogador=new Array();</script>"; // ***** 2022/03/17 *** Eliminar??? *** *****
	...
	echo "<script type='text/javascript'>
								...
								var QtEquipes=$QtEquipes;
								var TitCol=new Array ('Nr','Tit','Nome','FED','RtFIDE','RtNAC','Clube');
								";

								//RodadaMesaEquipe
								echo "RodadaMesaEquipe=new Array('".$NumRodadas."');";
								//echo "RodadaMesaEquipe[1]='HHHHH';";
								
								for($r=1;$r<=$NumRodadas;$r++)
								{
									echo "RodadaMesaEquipe['".$r."']=new Array('".$Mesa."');";
									
									for($Mesa=1;$Mesa<=$QtMesaRod[$r];$Mesa++)
									//for($Mesa=1;$Mesa<=5             ;$Mesa++)
									{
										//echo "RodadaMesaEquipe['".$r."']['".$Mesa."']='Mesa';";

									  echo "RodadaMesaEquipe['".$r."']['".$Mesa."']=new Array(1);"; // ***** 2022/03/10 *****
										echo "RodadaMesaEquipe['".$r."']['".$Mesa."'][1]='".$RodMesaEquip[$r][$Mesa][1][1]."';";
										echo "RodadaMesaEquipe['".$r."']['".$Mesa."'][2]='".$RodMesaEquip[$r][$Mesa][2][1]."';";
										
										//echo "RodadaMesaEquipe[1       ][1          ]='Mesa';";
										
										//echo "RodadaMesaEquipe[".$r."][".$Mesa."]=new Array();";
										//echo "RodadaMesaEquipe[".$r."][".$Mesa."][]=new Array();";
										//echo "RodadaMesaEquipe[".$r."][".$Mesa."][][]=new Array();";
										//echo "RodadaMesaEquipe[".$r."][".$Mesa."]=".$RodMesaEquip[$r][$Mesa][1][0].";";
										
										/*
										echo "RodadaMesaEquipe[".$r."][".$Mesa."]"."[1][0]=".$RodMesaEquip[$ir][$Mesa][1][0].";";
										echo "RodadaMesaEquipe[".$r."][".$Mesa."]"."[1][1]=".$PontosEquipeRod[$RodMesaEquip[$ir][$Mesa][1][0]][$ir].";";
										echo "RodadaMesaEquipe[".$r."][".$Mesa."]"."[2][0]=".$RodMesaEquip[$ir][$Mesa][2][0].";";
										echo "RodadaMesaEquipe[".$r."][".$Mesa."]"."[2][1]=".$PontosEquipeRod[$RodMesaEquip[$ir][$Mesa][2][0]][$ir].";";
										*/
									}
								}
	...
	echo "<script type='text/javascript'>
					...
					QtJogMesaEquip='".$QtJogMesaEquip."';

					function ImprimirRodadas()
					{
						if(QtEquipes<1)
						{Conteudo=ImprimirRodadasIndiv();}
						else
						{Conteudo=ImprimirRodadasEquipes();}
						return Conteudo;
					}

								function ImprimirRodadasIndiv()
								{
									...
									for(r=1;r<=NumRodadas;r++)
									{
										...
										for (m=1;m<=TabulRodada[r];m++)
										{
											...
											{	
												...
												//Conteudo=Conteudo+jogador[RodadaMesa[r][m].NumIniJogadorB-1].NomeJogador; ***** 2022/02/28 *****
												if (RodadaMesa[r][m].NumIniJogadorB<1)
												{
													Conteudo=Conteudo+'&nbsp;';
													Conteudo=Conteudo+'</span></a>';
													Conteudo=Conteudo+'</td>';
							...
							function ImprimirRodadasEquipes() //pauta
  {
	//Tab  FED  Tit  Rat  Nome  Pts  Res  Pts  Nome  Rat  Tit  FED
	Conteudo='';
	...
	for(r=1;r<=NumRodadas;r++)
	{
		...
		zMesa=0;
		//QtJogMesaEquip=2;
		//var Tabul_por_Mesa=4; // ***** 2022/03/17 *** definir Qt. tabul. por Mesa/Equipe *** ****
		//var Tabul_da_Mesa=5;

		Tabul_por_Mesa=QtJogMesaEquip; // ***** 2022/03/18 ****
		Tabul_da_Mesa=Tabul_por_Mesa + 1;

		for (t=1;t<=TabulRodada[r];t++)
		{

			if (Tabul_da_Mesa<Tabul_por_Mesa)
			{
				Tabul_da_Mesa=Tabul_da_Mesa+1;
			}
			else
			{
			 Tabul_da_Mesa=1;
			 ...
			 Conteudo=Conteudo+'<th colspan=3>Mesa: '+zMesa+'</th>';
			 // Equipe1;
  		 Conteudo=Conteudo+'<th align=left>'+RodadaMesaEquipe[r][zMesa][1]+'</th>';
			 Conteudo=Conteudo+'<th>Pts</th>';
			 Conteudo=Conteudo+'<th></th>';
			 Conteudo=Conteudo+'<th>Pts</th>';
			 // Equipe2;
  		 Conteudo=Conteudo+'<th align=left>'+RodadaMesaEquipe[r][zMesa][2]+'</th>';
			 Conteudo=Conteudo+'<th colspan=3><center></th>';
			 Conteudo=Conteudo+'</tr>';
			 //***
			...
			if(t % 2 == 0)
				...
				{Conteudo=Conteudo+' <tr bgcolor=\"#ffffff\">';}
			Conteudo=Conteudo+'<td>'+t+'</td>';
		
			Conteudo=Conteudo+'<td><center>';
			//Conteudo=Conteudo+'<span style=\"font-family:Consolas,Helvetica,sans-serif;font-size:15px;\">'+RodadaMesa[r][t].PaisJogadorA+'</span>';
			//flag
			Conteudo=Conteudo+' <div class=\"flag\" style=\"position:relative;float:center;\">';
			Conteudo=Conteudo+' <div title='+RodadaMesa[r][t].PaisJogadorA+' class=\"tn_'+RodadaMesa[r][t].PaisJogadorA+'\"></div>';
			Conteudo=Conteudo+' </div>';

			Conteudo=Conteudo+'</td>';
			
			Conteudo=Conteudo+'<td>';
			Conteudo=Conteudo+'<span style=\"font-family:Consolas,Helvetica,sans-serif;font-size:15px;\">'+RodadaMesa[r][t].TitJogadorA+'</span>';
			Conteudo=Conteudo+'</td>';
			Conteudo=Conteudo+'<td>'+RodadaMesa[r][t].RatJogadorA+'</td>';
			Conteudo=Conteudo+'<td><a href=\"#\"><span style=\"color:blue;\" onClick=\"pegarPosClick(event);mudarFichaJogador('+(RodadaMesa[r][t].NumIniJogadorA-1)+');\">';
			
			
			Conteudo=Conteudo+RodadaMesa[r][t].NomeJogadorA;
			//Conteudo=Conteudo+jogador[RodadaMesa[r][t].NumIniJogadorA-1].NomeJogador;


			Conteudo=Conteudo+'</span></a> ('+RodadaMesa[r][t].NumIniJogadorA+')';
			Conteudo=Conteudo+'</span></a> ('+RodadaMesa[r][t].EquipeJogadorA+')'; // ***** Temporario *** 2022/03/05 *****

			Conteudo=Conteudo+'</td>';
			
			Conteudo=Conteudo+'<td>';
			Conteudo=Conteudo+RodadaMesa[r][t].PontosJogadorA;
			Conteudo=Conteudo+'</td>';
			Conteudo=Conteudo+'<td><center>';
			Conteudo=Conteudo+RodadaMesa[r][t].ResPartida;
			Conteudo=Conteudo+'</center></td>';
			Conteudo=Conteudo+'<td>';
			Conteudo=Conteudo+RodadaMesa[r][t].PontosJogadorB;
			Conteudo=Conteudo+'</td>';
			
			if (RodadaMesa[r][t].NomeJogadorB!='Bye')
			{	
				Conteudo=Conteudo+'<td><a href=\"#\"><span style=\"color:blue;\" onClick=\"pegarPosClick(event);mudarFichaJogador('+(RodadaMesa[r][t].NumIniJogadorB-1)+');\">';
				
				//Conteudo=Conteudo+RodadaMesa[r][t].NomeJogadorB;
				
				
				//Conteudo=Conteudo+jogador[RodadaMesa[r][t].NumIniJogadorB-1].NomeJogador; ***** 2022/02/28 *****
				if (RodadaMesa[r][t].NumIniJogadorB<1)
				{
					Conteudo=Conteudo+'&nbsp;';
					Conteudo=Conteudo+'</span></a>';
					Conteudo=Conteudo+'</td>';
					...
					Conteudo=Conteudo+'&nbsp;';
					//Conteudo=Conteudo+'<span style=\"font-family:Consolas,Helvetica,sans-serif;font-size:15px;\">'+RodadaMesa[r][t].TitJogadorB+'</span>';
					
					Conteudo=Conteudo+'</td>';
					Conteudo=Conteudo+'<td><center>';
					//Conteudo=Conteudo+'<span style=\"font-family:Consolas,Helvetica,sans-serif;font-size:15px;\">'+RodadaMesa[r][t].PaisJogadorB+'</span>';
					//flag
					Conteudo=Conteudo+' <div class=\"flag\" style=\"position:relative;float:center;\">';
					Conteudo=Conteudo+' <div title='+RodadaMesa[r][t].PaisJogadorB+' class=\"tn_'+RodadaMesa[r][t].PaisJogadorB+'\"></div>';

				}
				else
				{
					Conteudo=Conteudo+jogador[RodadaMesa[r][t].NumIniJogadorB-1].NomeJogador;
					Conteudo=Conteudo+'</span></a> ('+RodadaMesa[r][t].NumIniJogadorB+')';

					Conteudo=Conteudo+'</span></a> ('+RodadaMesa[r][t].EquipeJogadorB+')'; // ***** Temporario *** 2022/03/05 *****

					Conteudo=Conteudo+'</td>';
					Conteudo=Conteudo+'<td>';
					Conteudo=Conteudo+RodadaMesa[r][t].RatJogadorB;
					Conteudo=Conteudo+'</td>';
					Conteudo=Conteudo+'<td>';
					Conteudo=Conteudo+'<span style=\"font-family:Consolas,Helvetica,sans-serif;font-size:15px;\">'+RodadaMesa[r][t].TitJogadorB+'</span>';
					Conteudo=Conteudo+'</td>';
			...
			{												
				if (RodadaMesa[r][t].NomeJogadorB==-1)
				{Conteudo=Conteudo+'<td>Bye</td>';}
				else
				{
					if (RodadaMesa[r][t].ResPartida=='½x-')
					{Conteudo=Conteudo+'<td>Bye Ausente</td>';}
					else
					{Conteudo=Conteudo+'<td>Não emparceirado</td>';}
				}
...
echo "</script>";
								...
								//echo "<script language='javascript' type='text/javascript'>"; // ***** 2022/03/10 *****
								echo "<script type='text/javascript'>"; // ***** 2022/03/10 *****
								echo "
		...
		echo "<script type='text/javascript'>";
			...
			for($r=1;$r<=$NumRodadas;$r++)
			{
				...
				for ($m=1;$m<=$TabulRodada[$r];$m++)
				{
					...
					echo "RodadaMesa[".$r."][".$m."].EquipeJogadorA='".$RodMesaResult[$r][$m][14]."';"; // ***** 2022/03/05 *****
					echo "RodadaMesa[".$r."][".$m."].EquipeJogadorB='".$RodMesaResult[$r][$m][15]."';"; // ***** 2022/03/05 *****
				}
			}
...
// ------------------------------------------------------------------------													
				...
				if($apresentacao=='geral')
				{
					echo " <b>SubTítulo</b>: $SubTituloTorneio<br>
												...
												Arquivo do Torneio: $ArquivoSMx<br>
	...
	//divListaClassif tab_classif divMostrarListaClassif
		echo '<div id="divListaClassif" name="divListaClassif" style="border=12px; position:absolute;width:99%;overflow:hidden;visibility:visible;height:0px;border:1px solid #2266AA;">';
    echo ' <b>Lista de Classificação</b>: ';
		echo " <div id='tab_classif' name='tab_classif' style='border=9px; bordercolor=\"red\"; width:800px;position:relative;overflow:hidden;visibility:visible;height:0px;'>";
		...
		echo 'document.getElementById("tab_classif").style.width=300px'; //document.getElementById("table_Classif").offsetWidth;';
											...
											if(count($PontosJogador)>0)
												{
													...
													*/
													...
													CalcularDesempates();
													//echo "<br><br>".count($PtsDesemp). " - Levy: t01 <br><br>";//exit;
													...
													*/
													...
													//echo "<br><br>".$QtJogadores. " - Levy: t02 <br><br>";//exit;
...
// ------------------------------------------------------------------------													
			// impimir Tabela de Classificação
			echo "<script type='text/javascript'>";
			...
			//echo 'document.getElementById("tab_classif").innerHTML=ImprimirClassif("");';
			echo 'document.getElementById("tab_classif").innerHTML="texto de teste;';
			//echo "alert(\" vvvv \")";
...
// ------------------------------------------------------------------------													
			...
			echo "</script>";
					...
					//echo "<script language='javascript' type='text/javascript'>
					echo "<script type='text/javascript'>
					JogClassif=new Array();performace=new Array();ratvart=new Array();
					</script>";
					for ($jpt=1;$jpt<=$QtJogadores;$jpt++)
					{
						//echo "<script language='javascript' type='text/javascript'>
...
//				$VlrInt = pack("n", 298);
	...
	function LerCampoIntTest($Int2)
			{
				$VlrInt = $Int2; //strrev(fread($PointerFile, 2));
				$arr = unpack("n", $VlrInt);				// ***** 2019/10/26 *****
				$value = $arr[1];
				return $value;				
			}
  ...
  function LerCampoFloat($strfloat)
	...
	function LerSeqStr($PointerFile,$QtBytes)
	{
		$VrlAsc = "";
    for ($i=1;$i<=$QtBytes;$i++)
		{
			$VrlAsc = $VrlAsc . fread($PointerFile, 1);
		}
		return $VrlAsc;
	}
