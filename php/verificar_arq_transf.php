<?php 
 ...
 $gravar = $list_par[6];
	...
	$sobrescrever = $HTTP_POST_VARS['sobrescrever'];
			...
			switch ($gravar)
			 {
				 ...
				 case 'U':
					 //echo "case U";exit;
					 $sql = pg_query($conexao,"UPDATE torneios SET nome='$TituloTorneio',remetente='$remetente_atual',email='$email_atual',prog='$prog',data_rec='$DataRec',status='$status' WHERE arquivo='$nomedoarquivo'") or die("erro de SELECT");
						if (copy($dir_orig . $nomedoarquivo, $dir_dest . $nomedoarquivo))
						 {
								unlink($dir_orig . $nomedoarquivo);
							}
