...
//			while ($i<$resultado)
			while ($i<1)
			{
				...
				$tabela='r'.$data_baseL;
				$sqlexprC = "ALTER TABLE " . $tabela . " ADD COLUMN titulo character(3) ";	//*2015/10/31*
				$sqlC=pg_query($conexaoL,$sqlexprC) or die("Sem Cadastros 1!");
				$sqlexprC = "UPDATE " . $tabela . " SET titulo='   '";	//*2015/10/31*
				$sqlC=pg_query($conexaoL,$sqlexprC) or die("Sem Cadastros 2!");
				$sqlexprC = "ALTER TABLE " . $tabela . " ADD COLUMN status99 character(1) ";	//*2015/10/31*
				$sqlC=pg_query($conexaoL,$sqlexprC) or die("Sem Cadastros 3!");
				$sqlexprC = "ALTER TABLE " . $tabela . " ALTER COLUMN status99 SET DEFAULT 'A' ";	//*2015/10/31*
				$sqlC=pg_query($conexaoL,$sqlexprC) or die("Sem Cadastros 4!");
				$sqlexprC = "UPDATE " . $tabela . " SET status99=status";	//*2015/10/31*
				$sqlC=pg_query($conexaoL,$sqlexprC) or die("Sem Cadastros 5!");
				$sqlexprC = "ALTER TABLE " . $tabela . " DROP COLUMN status";	//*2015/10/31*
				$sqlC=pg_query($conexaoL,$sqlexprC) or die("Sem Cadastros 6!");
				$sqlexprC = "ALTER TABLE " . $tabela . " RENAME COLUMN status99 TO status";	//*2015/10/31*
				$sqlC=pg_query($conexaoL,$sqlexprC) or die("Sem Cadastros 7!");
				//echo "$sqlexprC";exit;
