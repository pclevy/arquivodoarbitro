<?php
	...
	// * 2014/08/14 * 2014/07/30 * 2013/06/09 * 2012/04/14 * 2012/01/31 *  //Conectando com o DB
	
	// Lendo String de conexão
	$file = "../config/conexao_ca.cfg";	// para Arquido do Árbitro
	$fh = fopen($file, 'r');
  ...
  $sql =pg_query($conexao,"SELECT reg,nome,titulo,rating,clube FROM tabela_ajustes ORDER BY reg") or die(); // Rating: 30/07/2014
		...
		while ($i<$resultado)
			{
				...
				$sobrenome="";	/***2014/08/14*** testar na próxima atualização ***
				$status="A";
				
				//$sqltrexp = "INSERT INTO cadastro (reg) VALUES (" . $i . ")";
				$sqltrexp = "INSERT INTO cadastro (reg,nome,titulo,rating,clube,status) VALUES ('" . $reg . "','" . $nome . "','" . $titulo . "','" . $rating . "','" . $clube . "','" . $status . "')";
				/*
				if($reg<3885)
					{
					 $sqltrexp = "UPDATE cadastro SET titulo='$titulo',clube='$clube',rating='$rating' WHERE reg='" . $reg . "'";
					 echo "<br>Reg: $i - $reg";					 
					}
				else
					{
